...
const DELIVERY_RECEIPT_TRIGGER_SQL = `CREATE TRIGGER IF NOT EXISTS record_delivery_receipt
...
CREATE TABLE IF NOT EXISTS delivery_receipts (
 ...
 monitor_name text NOT NULL, channel_name text NOT NULL,
 event_type text NOT NULL, delivered_at integer NOT NULL
);
CREATE TABLE IF NOT EXISTS revoked_sessions (id text PRIMARY KEY NOT NULL, expires_at integer NOT NULL);
...
export async function ensureSchema(d1: D1Database): Promise<void> {
  ...
  if (!migrationPromise) {
    migrationPromise = (async () => {
      ...
      await d1.prepare(DELIVERY_RECEIPT_TRIGGER_SQL).run()

      const alterStatements = [
        `ALTER TABLE scheduler_leases ADD COLUMN last_completed_at integer`,
        `ALTER TABLE scheduler_leases ADD COLUMN last_run_failed integer DEFAULT false NOT NULL`,
